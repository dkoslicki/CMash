...
rm TrainingDatabase.h5 2> /dev/null
rm TrainingDatabase.tst 2> /dev/null
/usr/bin/time python ${scriptsDir}/MakeStreamingDNADatabase.py filenames.txt TrainingDatabase.h5 --verbose
if test -f TrainingDatabase.h5; then
  if test -f TrainingDatabase.tst; then
...
rm results.csv 2> /dev/null
# make a streaming pre-filter
/usr/bin/time python ${scriptsDir}/StreamingQueryDNADatabase.py ${testOrganism} TrainingDatabase.h5 results.csv 10-21-2 -v
if test -f results.csv; then
...
# make a streaming pre-filter
/usr/bin/time python ${scriptsDir}/MakeStreamingPrefilter.py TrainingDatabase.h5 prefilter.bf 10-21-2
